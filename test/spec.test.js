...
test('F75: auditkit version comparison', () => {
    ...
    assert.strictEqual(versionAtLeast([0, 3, 4], MIN_AUDITKIT), false);
    assert.strictEqual(versionAtLeast([0, 3, 5], MIN_AUDITKIT), true);
...
test('F75: rigor mode rejects an auditkit older than the minimum', { skip: process.platform === 'win32' && 'uses a shell-script stub' }, () => {
    ...
    assert.strictEqual(result.ok, false);
    assert.match(result.report, /older than 0\.3\.5/);
});
