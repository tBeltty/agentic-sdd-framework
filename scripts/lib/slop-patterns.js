...
// sentences like "no es rápida porque es síncrona" are not reported.
const BANNED_PATTERNS = [
    ...
    {
        name: 'Binary Contrast ("no es X, es Y")',
        regex: /\bno es\b[^.;:!?]{1,60}?(?:[,;—–]\s*(?:sino\s+(?:que\s+)?)?es\b|,?\s+sino\b)/i
    },
    {
        name: 'Throat-Clearing Opener',
        regex: /\b(?:here['’]?s the thing|it['’]?s worth noting that|it is worth noting that|it is important to (?:note|remember) that|cabe destacar que|es importante (?:destacar|señalar) que|vale la pena mencionar que)\b/i
    },
    {
        name: 'Colon Reveal Hook',
        regex: /\b(?:the best part|here['’]?s the kicker|lo mejor)\s*:/i
    },
    {
        name: 'Faux-Insight Hook',
        regex: /\b(?:what nobody tells you|lo que nadie te dice)\b/i
    },
    {
